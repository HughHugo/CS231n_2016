...
class Solver(object):
    """
    A solver encapsulates all the logic necesary for training classification models. The Solver performs stochastic 
    gradient descent using different update rules defined in optim.py.
    
    The solver accepts both training and validation data and labels so it can periodically check classification 
    accuracy on both training and validation data to watch out for overfitting.
    
    To train a model, you will first construct a solver instance, passing the model, dataset, and various options 
    (learning rate, batch size, etc) to the constructor. You will then cal the train() method to run optimization
    procedure and train the model.
    
    After the train() method returns, model.params will contain the parameters that performed best on the validation
    set over the cource of training. In addition, the instance variable solver.loss_history will contain a list of 
    all losses encountered during training and the instance variables solver.train_acc_history and 
    solver.val_acc_history will be lists containing the accuracies of the model on the training and validation set at
    each epoch.
    ...
    - model.loss(x, y) must be a function that computes training-time loss and gradients, and test-time classification 
      ...
      - y: Array of labels, of shape (N,) giving labels for x where y[i] is the label of x[i]
      
      Returns:
      if y is None, run a test-time forward pass and return:
      - scores: Array of shape (N, C) giving classification scores for x where scores[i, c] gives the score of class 
                c for x[i].
      
      if u in not None, run a training time forward and backward pass and return a tuple of :
    ...
    def __init__(self, model, data, **kwargs):
        ...
        - data: A dictionary of training and validationdata with the following:
        ...
        - lr_decay: A scalar for learning rate decay; after each epoch the learning rate is pultiplied by this value.
    ...
    def check_accuracy(self, x, y, num_samples = None, batch_size = 100):
        ...
        - batch_size: Split x and y into batches of this size ot avoid using too much memory.
        
        Returns:
        - acc: Scalar gibing the fraction of instances that were correctly classified by the model.
        """
    ...
    def train(self):
        ...
        for t in range(num_iterations):
            ...
            # Check train and val accuracy on the first iteration, the last iteration, and the end of each epoch.
            first_it = (t == 0)
        ...
        # A the end of training swap the best params into the model.
